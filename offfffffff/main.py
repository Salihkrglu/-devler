...
def urun_kaydet():
    if "ad" in session and session["ad"]:        
        ...
        İSBN = request.form["İSBN"]
        isim = request.form["isim"]
        fiyat = request.form["fiyat"]

        baglanti = sqlite3.connect("veriler.db")
        sorgu = f"UPDATE urunler SET kod='{kod}',İSBN='{İSBN}', ad='{isim}', fiyat={float(fiyat)}  WHERE id={int(id)}"
        imlec = baglanti.cursor()
        imlec.execute(sorgu)
        # sorgu sonucu gelen kayitlar
        kayit = imlec.fetchone()
...
def urun_ekle():
    if "ad" in session and session["ad"]:
        if request.method == "POST":      
            ...
            baglanti = sqlite3.connect("veriler.db")
            sorgu  = f"INSERT INTO urunler (kod, İSBN, isim, fiyat, resim) VALUES ('{kod}','{İSBN}', '{isim}', {float(fiyat)}, '{resim.filename}')"
            imlec = baglanti.cursor()
            imlec.execute(sorgu)
            # sorgu sonucu gelen kayitlar
            kayit = imlec.fetchone()
